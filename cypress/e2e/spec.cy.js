...
describe('template spec', () => {
  ...
  it.skip('logs into TDG and generates data using a custom template', () => {
  ...
  //     // the new file would only show if we waited a few secs, navigated away and back to history (anyone else have this issue?)
    ...
    it("Logs into TDG and generates 100 data examples for firstName, lastName, email, fullAddress", () => {
      ...
      cy.get('#file-name-input').invoke("val").then((value) => {
        ...
        const zipFilePath = `cypress/downloads/${fileName}.zip`;
        ...
        cy.task('unzipFile', { zipFilePath, extractionPath }).then(() => {
          ...
          cy.readFile(extractionPath + '/JSON1.json').then((fileContent) => { // make JSON1.json not hard coded! (cy.latestFile?)
            ...
            console.log('Length of Array: ', fileContent.length); 
            cy.wrap(fileContent.length).should('eq', 100); 
            // could also check whether the required fields are there. 
          });
    });
      });
      });
